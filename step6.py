...
odir="figs"
plt.plot(phi_init)
plt.savefig('%s/phi_init_rank%d.png'%(odir,rank))
plt.close()

# Lets try and step forward
phi_x=np.zeros(myl+nh*2)
phi_xx=np.zeros(myl+nh*2)
kappa_arr=np.zeros(myl+nh*2)+dx**2/dt*0.5
phi_arr=phi_init
for i in [*range(1,500)]:
 phi_x[0:-1]=(phi_arr[1:]-phi_arr[0:-1])*rdx
 phi_xx[i0sec_indices]=kappa_arr[i0sec_indices]*(phi_x[1:-1]-phi_x[0:-2])*rdx
 halo(phi_xx,i0sec_indices,i0rm1,i0rp1,i0hih_indices,i0loh_indices,nh)
 phi_arr=phi_arr+phi_xx*dt
 if i%20 == 0:
   plt.plot(phi_init,'.')
   plt.plot(phi_arr)
   plt.savefig('%s/phi_arr_rank%d_iter%d.png'%(odir,rank,i))
   plt.close()
